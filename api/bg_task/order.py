...
async def update_order(ctx, status: ORDER_STATUS, order_number: str):
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            if not order:
                logger.warning(f"Background task update_order: No order found for {order_number}")
            ...
            # send payment receive email here
            arq = await get_arq_pool()
            if status == ORDER_STATUS.PROCESSING and len(order.vendors) < 1:
                await arq.enqueue_job("create_vendor_orders", order_number, _queue_name="16vmart")
            await db.commit()
            logger.info(f"Background task updated order {order_number} to PROCESSING")
            return {"success": True}
        except Exception as e:
            logger.exception(f"Failed to update order for {order_number}: {e}")
            await db.rollback()
...
async def create_vendor_orders(ctx, order_number: str):
    async with AsyncSessionLocal() as db:
        try:
            ...
            if not order:
                logger.warning(f"Background task create_vendor_orders: Order {order_number} not found")
            ...
            await db.commit()
            logger.info(f"Vendor orders created successfully for {order_number}")
            return {"success": True}

        except Exception as e:
            await db.rollback()
            logger.exception(f"Failed creating vendor orders for {order_number}: {e}")
