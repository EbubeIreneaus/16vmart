...
async def signup(
    ...
    try:
        ...
        try:
            ...
            await redis.set(f"16vmart:session:{session.id}", session_json, ex=ACCESS_TOKEN_EXP)
        except Exception as err:
            logger.warning(f"Failed to cache session in Redis for {email}: {err}")

        arq = await get_arq_pool()
        await arq.enqueue_job("update_session", new_session.id, user_agent, _queue_name="16vmart")
        await arq.enqueue_job("send_welcome_email", email, body.fullname, _queue_name="16vmart")
        ...
        logger.exception(f"Unexpected error during signup for {body.email}: {e}")
        raise HTTPException(
...
async def signin(
    ...
    try:
        ...
        arq = await get_arq_pool()
        await arq.enqueue_job("update_session", session.id, user_agent, _queue_name="16vmart")
        ...
        logger.exception(f"Unexpected error during signin for {body.email}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error, Try again later",
        )
...
async def signout(
    ...
    if refresh_token:
        try:
            ...
            if session:
                await redis.delete(f"16vmart:session:{session.id}")
...
async def reset_password(
    ...
    await redis.delete(f"16vmart:passwordReset:{ip}:{key_url}")
