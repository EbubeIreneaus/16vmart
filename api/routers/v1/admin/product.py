...
async def admin_create_category(
    ...
    try:
        category_name = body.name.lower().strip()
        slug = slugify(category_name, allow_unicode=True, lowercase=True)

        existing_cat = await db.scalar(
            select(Category).where(func.lower(Category.slug) == slug)
        )
        if existing_cat:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Category with name already exist",
            )

        new_category = Category(name=category_name, slug=slug)

        for key in body.attributes:
        ...
        if len(body.sub_categories) > 0:
            for sub_category in body.sub_categories:
                sub_c_name = sub_category.name.lower().strip()
                sub_slug = slugify(
                    f"{category_name}-{sub_c_name}",
                    allow_unicode=True,
                    lowercase=True,
                )

                sub_c = Category(name=sub_c_name, slug=sub_slug)

                if len(sub_category.attributes) > 0:
        ...
        db.add(new_category)
        await db.commit()
        await redis.delete("16vmart:cat:all")
        logger.info(f"Admin {user.id} created category {category_name} ({slug})")
        return {"success": True}

    except HTTPException:
        raise
    except Exception as e:
        logger.exception(f"Error creating category {body.name}: {e}")
        raise HTTPException(
