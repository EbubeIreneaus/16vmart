...
@limiter.limit("120/minute", error_message="Too many requests, try again later")
...
@limiter.limit("120/minute", error_message="Too many requests, try again later")
async def fetch_featured_product(request: Request, db: AsyncSession = Depends(get_db)):
    stmt = (
        select(Product)
        .distinct()
...
async def search_product(
    request: Request, s: str = Query(min_length=3), db: AsyncSession = Depends(get_db)
):
...
@limiter.limit("120/minute", error_message="Too many requests, try again later")
async def get_single_product(request: Request, slug: str, db: AsyncSession = Depends(get_db)):
    _slug = slug.lower().strip()
    cache = await redis.get(f"16vmart:product:{_slug}")
    if cache:
    ...
    await redis.set(f"16vmart:product:{_slug}", json_string, ex=86400)
    return _product


@router.get("/cat/{slug}", response_model=Page[MiniProductResponse])
@limiter.limit("120/minute", error_message="Too many requests, try again later")
async def get_product_by_category(request: Request, slug: str, db: AsyncSession = Depends(get_db)):
    stmt = (
        select(Product)
        .join(Product.category)
        .join(Product.store)
