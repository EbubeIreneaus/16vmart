...
async def create_store_profile(
    ...
    try:
        ...
        db.add(store)
        await db.flush()
        await redis.delete(f"16vmart:user:{_user.id}")
        logger.info(f"Store created successfully: {slug} by user {_user.id}")
        ...
        logger.exception(f"Error creating store profile for user {_user.id}: {e}")
        raise HTTPException(
...
@limiter.limit("10/minute", error_message="Too many requests, try again later")
async def update_business_logo(
    ...
    if res["secure_url"]:
        ...
        logger.info(f"Updated logo for store {store.slug}")
...
async def update_store_profile(
    ...
    logger.info(f"Updated store profile for {_store.slug}")
    return {"success": True}
...
async def delete_store_profile(
    ...
    await redis.delete(f"16vmart:store:{user.id}:{_store.slug}")
    logger.info(f"Deleted store profile for {_store.slug}")
