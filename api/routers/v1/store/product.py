...
async def create_product(
    ...
    try:
        ...
        logger.exception(f"Error creating product for store {store.slug}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.patch("/images/{name}/{product_id}")
@limiter.limit("20/minute", error_message="Too many requests, try again later")
async def upload_product_images(
    ...
    try:
        ...
        product = await db.scalar(
            select(Product)
            .options(
                selectinload(Product.images), selectinload(Product.store)
            )
            .where(
        ...
        await redis.delete(f"16vmart:product:{slug}")
        return {"success": True, "data": urls}

    except HTTPException:
        raise
    except Exception as e:
        logger.exception(f"Error uploading product image for {name}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Unknown server error, try again later",
        )
...
async def delete_product_image(
    ...
    try:
        ...
        logger.exception(f"Error deleting product image {name}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Unknown server error, try again later",
        )
...
async def edit_product(
    ...
    try:
        ...
        logger.exception(f"Error editing product {product_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Unknown server error, try again later",
        )
...
@limiter.limit("15/minute", error_message="Too many requests, try again later")
async def delete_product(
    ...
    await redis.delete(f"16vmart:product:{slug}")
    logger.info(f"Deleted product {slug} for store {store.slug}")
    return {"success": True}
...
async def get_all_product(
    ...
    try:
        ...
        logger.exception(f"Error fetching all products for store {store.slug}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Unknown server error, try again later",
        )
