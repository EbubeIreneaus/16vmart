...
async def delete_address(
    ...
    logger.info(f"User {user.id} deleted address {address_id}")
    return {"success": True}

@router.get("/address", response_model=List[AddressOutschema])
@limiter.limit("60/minute", error_message="Too many request, try again later")
...
@router.get("/address/{address_id}", response_model=AddressOutschema)
@limiter.limit("60/minute", error_message="Too many request, try again later")
