...
<template>
  <div class="min-h-screen bg-slate-100 lg:flex">
    <aside class="w-full bg-slate-950 p-5 text-slate-300 lg:fixed lg:h-screen lg:w-64">
      <AppLogo />
      <div class="mt-8 rounded-xl bg-slate-900 p-3">
        <p class="text-xs text-slate-500">Signed in as</p>
        <p class="mt-1 text-sm font-bold capitalize text-white">{{ auth.user?.role || 'admin' }}</p>
      </div>
      <nav class="mt-5 flex gap-1 overflow-auto lg:block">
        <NuxtLink
          v-for="link in links"
          :key="link.to"
          :to="link.to"
