    ...
    <div class="flex flex-wrap items-end justify-between gap-5">
      <div>
        ...
        <p class="mt-2 text-slate-600">
          Choose a store to manage its catalogue, orders, payouts, and profile.
        </p>
      </div>
      <NuxtLink
        to="/vendor/create"
    ...
    <div class="mt-8 grid gap-5 md:grid-cols-2" v-if="stores">
      ...
      >
        <div class="flex items-start justify-between gap-4">
          <div
            class="grid size-12 place-items-center rounded-xl bg-teal-100 text-xl font-black text-teal-800"
          >
        ...
        <p class="mt-1 text-sm text-slate-500">
          {{ store.industry }} · {{ store.city }}, {{ store.state }}
        </p>
        <div
          class="mt-6 flex items-center justify-between border-t border-slate-100 pt-4 text-sm"
        >
          <span class="text-slate-500">/{{ store.slug }}</span
          ><span class="font-bold text-teal-700">Manage store →</span>
        </div>
      </NuxtLink>
      <NuxtLink
        to="/vendor/create"
      ...
      >
        <div>
          <span
            class="mx-auto grid size-12 place-items-center rounded-full bg-white text-2xl text-teal-700 shadow-sm"
            >+</span
          >
          <h2 class="mt-4 text-xl font-black">Create another store</h2>
          <p class="mt-2 text-sm text-slate-500">
            Your account can own and manage multiple stores.
          </p>
        </div>
      </NuxtLink>
    </div>
